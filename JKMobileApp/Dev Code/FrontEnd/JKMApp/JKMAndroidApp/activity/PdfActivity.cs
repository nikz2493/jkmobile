...
namespace JKMAndroidApp.activity
{
    ...
    public class PdfActivity : Activity
    {
        ...
        // <summary>
        /// Event Name      : OnRequestPermissionsResult
        /// Author          : Sanket Prajapati
        /// Creation Date   : 23 jan 2018
        /// Purpose         : Get request permission result and create pdf
        /// Revision        : 
        /// </summary>
        public override void OnRequestPermissionsResult(int requestCode, string[] permissions, Permission[] grantResults)
        {
            if (requestCode == 111)
            {
                ...
                Log.Info(StringResource.msgDebugTagActivityMain, StringResource.msgRequestPermission);

                // Check if the only required permission has been granted
                if (grantResults.Length == 1 && grantResults[0] == Permission.Granted)
                {
                    // permission has been granted, preview can be displayed
                    Log.Info(StringResource.msgDebugTagActivityMain, StringResource.msgPermissionGranted);
                    ConvertBase64ToPDF();

                }
                else
                {
                    Log.Info(StringResource.msgDebugTagActivityMain, StringResource.msgPermissionNotGranted);
                }
            }
            else
            {
